...
# 体温范围
data.update({'tw': AskInteractive('今日体温范围(℃): ',
                                  ['(-inf, 36]', '(36, 36.5]', '(36.5, 36.9]', '(36.9, 37.3]', '(37.3, 38]',
                                   '(38, 38.5]', '(38.5, 39]', '(39, 40]', '(40, +inf)'], 0)})
...
# 其他信息
data.update({'qtqk': AskText('其他信息，回车为空: ')})

# 邮箱信息
data.update({'receive_email': AskText('收信邮箱名称(xxx@qq.com)，推荐QQ邮箱可以集是获得提醒: ')})
data.update({'sent_email': AskText('发信邮箱名称(xxx@xxx.com)，可以直接回车用我的: ')})
data.update({'sent_email_code': AskText('该邮箱的授权密码，非登录密码，可以直接回车用我的: ')})
data.update(({'sent_email_server': AskText('发送邮箱的SMTP服务器(smtp.163.com)，可以直接回车用我的: ')}))

name = AskText('保存json的名称(xxx)，确保不和别人重复即可: ')
if name == '':
    name = 'xxx'
